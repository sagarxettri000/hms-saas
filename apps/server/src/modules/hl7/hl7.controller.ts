...
export class Hl7Controller {
  ...
  @ApiOperation({
    summary: 'Ingest a raw HL7 v2 message (ADT/ORM/ORU/SIU) - async queued processing',
  })
  @ApiQuery({ name: 'async', required: false, type: Boolean, description: 'Process asynchronously (default: true)' })
  async ingestRaw(
    @Body() body: string,
    ...
    if (isAsync) {
      const jobId = await this.hl7Queue.enqueueMessage(String(body ?? ''), {
  ...
  @HttpCode(202)
  @Permissions(PermissionAction.CREATE)
  @ApiOperation({ summary: 'Ingest an HL7 v2 message wrapped in JSON { message } - async queued' })
  @ApiQuery({ name: 'async', required: false, type: Boolean, description: 'Process asynchronously (default: true)' })
  async ingestJson(
    ...
    if (isAsync) {
      const jobId = await this.hl7Queue.enqueueMessage(body?.message ?? '', {
        tenantId: user.tenantId,
        userId: user.id,
      }, 'http');
      return { accepted: true, jobId, status: 'queued' };
