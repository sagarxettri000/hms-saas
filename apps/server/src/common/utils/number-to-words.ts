...
const isWhole = (n: number): boolean => Number.isFinite(n) && Math.floor(Math.abs(n)) === Math.abs(n);
 ...
 */
export function amountInWords(amount: number | string, currency = 'NPR'): string {
