...
function assemblePdf(pages: PdfPage[]): Buffer {
  ...
  for (const p of pages) {
    ...
    const contentIdx = addObj(
      `<< /Length ${Buffer.byteLength(stream, "latin1")} >>\nstream\n${stream}\nendstream`,
    );
    const fontIdx = addObj(
  ...
  for (const p of pages) {
    ...
    add2(
      `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_W} ${PAGE_H}] /Contents ${contentNum} 0 R /Resources << /Font << /F1 ${fontNum} 0 R >> >> >>`,
    );
  }
  ...
  for (const off of offsets2)
    pdf2 += `${String(off).padStart(10, "0")} 00000 n \n`;
...
export function buildLabReportPdf(
  ...
  const regLine = [
    hospital.panNumber ? `PAN: ${hospital.panNumber}` : "",
    hospital.vatNumber ? `VAT: ${hospital.vatNumber}` : "",
    hospital.registrationNumber ? `Reg: ${hospital.registrationNumber}` : "",
  ]
    .filter(Boolean)
    .join("  |  ");
  if (regLine) {
    p.text(MARGIN, p.y, regLine, 8, "0.4 0.4 0.4");
    p.gap(10);
  }
  ...
  p.text(
    MARGIN,
    p.y,
    order.reportTitle || "LABORATORY REPORT",
    14,
    "0.12 0.24 0.4",
  );
  if (order.department) {
  ...
  if (order.verifiedAt)
    infoRow("Verified:", dateTimeStr(order.verifiedAt), MARGIN + 260, p.y);
  p.gap(14);
  if (order.processedAt) {
    infoRow("Processed:", dateTimeStr(order.processedAt), MARGIN, p.y);
    p.gap(14);
  }
  if (order.approvedAt) {
  ...
  if (order.reportedAt) {
    infoRow("Reported:", dateTimeStr(order.reportedAt), MARGIN, p.y);
    p.gap(14);
  }
  ...
  if (order.patient.hospitalNumber)
    infoRow("Hosp. #:", order.patient.hospitalNumber, MARGIN + 260, p.y);
  p.gap(14);
  if (order.patient.mrn) infoRow("MRN:", order.patient.mrn, MARGIN, p.y);
  if (order.patient.dateOfBirth)
    infoRow("DOB:", dateStr(order.patient.dateOfBirth), MARGIN + 260, p.y);
  p.gap(14);
  if (order.patient.gender)
    infoRow("Gender:", order.patient.gender, MARGIN, p.y);
  p.gap(14);
  if (order.doctor?.user) {
    const docName = [
      order.doctor.user.firstName,
      order.doctor.user.middleName,
  ...
  if (order.samples && order.samples.length > 0) {
    ...
    for (const s of order.samples) {
      const parts = [
        s.specimenType,
        s.collectedAt ? `Collected: ${dateStr(s.collectedAt)}` : "",
        `Status: ${s.status}`,
      ].filter(Boolean);
      ...
      if (s.barcode) {
        ...
        p.text(
          MARGIN + 6,
          p.y + 6,
          s.barcode.padEnd(14, " ").split("").join(" "),
          8,
        );
  ...
  for (const item of order.items) {
    ...
    const name =
      item.testName.length > 20
        ? item.testName.slice(0, 18) + ".."
        : item.testName;
    ...
    const resultStr = !hasResult
      ...
      : item.resultValue != null
        ? fmtNum(item.resultValue, item.precision)
        : item.result || "—";
    const flagColor = item.isCritical
      ? "0.8 0.1 0.1"
      : item.isAbnormal
        ? "0.8 0.5 0"
        : "0.2 0.6 0.3";
  ...
  p.text(
    MARGIN,
    p.y,
    `Reviewed and approved for ${hospital.name}`,
    9,
    "0.3 0.3 0.3",
  );
  if (order.approvedByName) {
    p.text(
      right,
      p.y,
      `Electronically signed by: ${order.approvedByName}`,
      8,
      "0.4 0.4 0.4",
    );
  } else if (generatedBy) {
