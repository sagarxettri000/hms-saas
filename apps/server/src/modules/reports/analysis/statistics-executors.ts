...
function countByGender(encs: any[]): {
  male: number;
  female: number;
  other: number;
} {
...
export async function deptWiseStats(ctx: ExecContext): Promise<ExecResult> {
  ...
  const returning = await returningIds(
    ctx.prisma,
    ctx.tenantId,
    patientIds,
    ctx.filters.from,
  ...
  return {
    ...
    cards: [
      {
        ...
        value: rows.reduce((s, r) => s + r.totalPatients, 0),
      },
    ...
    chart: {
      ...
      series: [{ key: "totalPatients", label: "Patients" }],
    },
  };
}
...
export async function monthWiseStats(ctx: ExecContext): Promise<ExecResult> {
  ...
  if (ctx.filters.departmentId)
    admWhere.departmentId = ctx.filters.departmentId;
...
async function ageClassified(
  ...
  if (kind === "opd") {
    ...
    for (const e of encounters) {
      ...
      deptIds.add(deptId);
      const group =
        ageGroupOf(e.patient?.dateOfBirth) ||
        (e.patient?.age ? ageGroupOfAge(e.patient.age) : "Unknown");
    ...
    const admissions = await ctx.prisma.admission.findMany({
      where,
      include: {
        patient: { select: { gender: true, dateOfBirth: true, age: true } },
      },
    });
    for (const a of admissions) {
      ...
      deptIds.add(deptId);
      const group =
        ageGroupOf(a.patient?.dateOfBirth) ||
        (a.patient?.age ? ageGroupOfAge(a.patient.age) : "Unknown");
      const key = `${deptId}|${group}`;
      const cur = byKey.get(key) || {
        departmentId: deptId,
        ageGroup: group,
  ...
  const rows = [...byKey.values()].map((v) => ({
    department:
      v.departmentId === "none" ? "-" : deptMap.get(v.departmentId) || "-",
    ageGroup: v.ageGroup,
    male: v.male,
    female: v.female,
    other: v.other,
...
async function inpatientStats(ctx: ExecContext): Promise<ExecResult> {
  ...
  const deptIds = [
    ...new Set(admissions.map((a) => a.departmentId).filter(Boolean)),
  ] as string[];
  ...
  for (const a of admissions) {
    ...
    const end =
      a.dischargeDate && a.isDischarged ? a.dischargeDate : new Date();
  ...
  const rows = [...byDept.values()].map((v) => {
    ...
    const beds =
      bedMap.get(v.departmentId === "none" ? null : v.departmentId) || 0;
  ...
  return {
    ...
    cards: [
      {
        label: "Admissions",
        value: rows.reduce((s, r) => s + r.admissions, 0),
      },
      {
        label: "Discharges",
        value: rows.reduce((s, r) => s + r.discharges, 0),
      },
...
export function periodicalInpatientStats(
  ctx: ExecContext,
): Promise<ExecResult> {
  return inpatientStats(ctx);
}
...
export async function erStatistics(ctx: ExecContext): Promise<ExecResult> {
  ...
  const rows = cases.map((c) => {
    ...
    return {
      ...
      disposition: c.admitted
        ? c.admittedTo || "Admitted"
        : c.dischargedAt
          ? "Discharged"
          : "Under Care",
    };
    ...
    include: {
      ...
      bedAllocations: {
        ...
        select: {
          bedId: true,
          bed: {
            select: { ward: { select: { name: true, departmentId: true } } },
          },
        },
      },
    },
...
export async function patientAnalysisDoctor(
  ctx: ExecContext,
): Promise<ExecResult> {
  const encounters = await fetchEncounters(ctx);
  const patientIds = [...new Set(encounters.map((e) => e.patientId))];
...
export async function dischargeRecordSheet(
  ctx: ExecContext,
): Promise<ExecResult> {
  ...
  const admissions = await ctx.prisma.admission.findMany({
    ...
    include: {
      ...
      invoices: {
        select: { dueAmount: true, totalAmount: true, status: true },
      },
    },
  ...
  const rows = admissions.map((a) => {
    const outstanding = a.invoices.reduce(
      (s: number, i: any) => s + money(i.dueAmount),
      0,
    );
    return {
      ...
      doctor: a.admittingDoctorId
        ...
        : "-",
      diagnosis:
        a.finalDiagnosis || a.primaryDiagnosis || a.provisionalDiagnosis || "-",
      dischargeType: a.dischargeType || "-",
      billingStatus:
        outstanding > 0 ? "OUTSTANDING" : a.invoices.length ? "SETTLED" : "-",
    };
  });
...
export async function patientDetailOpd(ctx: ExecContext): Promise<ExecResult> {
  ...
  const docIds = [
    ...new Set(encounters.map((e) => e.doctorId).filter(Boolean)),
  ] as string[];
  const docMap = await doctorNames(ctx.prisma, ctx.tenantId, docIds);
  ...
  return {
    ...
    totals: {
      ...
      billing: r2(rows.reduce((s, r) => s + r.billing, 0)),
    },
...
export async function patientDetailIpd(ctx: ExecContext): Promise<ExecResult> {
  ...
  const rows = admissions.map((a) => {
    ...
    const services = a.invoices.reduce(
      (s: number, i: any) => s + (i.items || []).length,
      0,
    );
    return {
      patient: fullName(a.patient),
...
export async function bedOccupancy(ctx: ExecContext): Promise<ExecResult> {
  ...
  if (ctx.filters.from || ctx.filters.to) {
    ...
    if (ctx.filters.from)
      allocWhere.OR.push(
        { releasedAt: null },
        { releasedAt: { gte: ctx.filters.from } },
      );
    if (ctx.filters.to)
      allocWhere.OR.push({ allocatedAt: { lte: ctx.filters.to } });
  }
  ...
  const rows = beds.map((b) => {
    ...
    return {
      ...
      patient: alloc?.admission?.patient
        ? fullName(alloc.admission.patient)
        : "-",
      admissionDate: alloc?.admission?.admissionDate,
      duration: alloc?.admission?.admissionDate
        ? daysBetween(alloc.admission.admissionDate, new Date())
        : 0,
    };
  });
  ...
  return {
    ...
    cards: [
      {
        label: "Occupied Beds",
        value: rows.filter((r) => r.patient !== "-").length,
      },
...
export async function bedAnalysis(ctx: ExecContext): Promise<ExecResult> {
  ...
  for (const m of movements)
    movesByBed.set(m.bedId, (movesByBed.get(m.bedId) || 0) + 1);
  ...
  const rows = [...byBed.values()].map((v) => ({
    ...
    avgOccupancy: v.count
      ? Math.round((v.occupiedDays / v.count) * 10) / 10
      : 0,
  ...
  return {
    ...
    cards: [
      { label: "Beds Analyzed", value: rows.length },
      {
        label: "Occupied Days",
        value: rows.reduce((s, r) => s + r.occupiedDays, 0),
      },
    ],
  };
}
...
export async function monthlyDoctorWise(ctx: ExecContext): Promise<ExecResult> {
  ...
  for (const e of encounters) {
    ...
    const cur = byKey.get(key) || {
      month: monthLabel(monthKey(e.createdAt)),
      doctorId: e.doctorId,
      patients: 0,
      opd: 0,
      ipd: 0,
  ...
  const invWhere = invoiceWhere(ctx, {
    items: { some: { doctorId: { in: [...docIds] } } },
  });
  const invoices = await ctx.prisma.invoice.findMany({
    where: invWhere,
    select: {
      issuedDate: true,
      items: { select: { doctorId: true, lineTotal: true } },
    },
  });
  for (const inv of invoices) {
  ...
  return {
    ...
    cards: [
      { label: "Doctors", value: docIds.size },
      { label: "Revenue", value: r2(t("revenue")) },
    ],
  };
}

export async function docWisePatientTotal(
  ctx: ExecContext,
): Promise<ExecResult> {
  const encounters = await fetchEncounters(ctx);
  const patientIds = [...new Set(encounters.map((e) => e.patientId))];
  ...
  for (const e of encounters) {
    if (!e.doctorId) continue;
    docIds.add(e.doctorId);
    const cur = byDoc.get(e.doctorId) || {
      doctorId: e.doctorId,
      opd: 0,
      ipd: 0,
      newP: 0,
...
export async function ageWiseOpd(ctx: ExecContext): Promise<ExecResult> {
  ...
  return {
    ...
    cards: [
      { label: "OPD Patients", value: rows.reduce((s, r) => s + r.count, 0) },
    ],
  };
}

export async function doctorWiseReferral(
  ctx: ExecContext,
): Promise<ExecResult> {
  ...
  const rows = admissions.map((a) => ({
    ...
    receivingDoctor: a.admittingDoctorId
      ? docMap.get(a.admittingDoctorId) || "-"
      : "-",
