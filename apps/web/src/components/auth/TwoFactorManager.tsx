...
export default function TwoFactorManager() {
  ...
  return (
    ...
    >
      <h2 style={{ fontSize: 15, fontWeight: 600, margin: '0 0 4px', color: 'var(--text)' }}>
        Two-factor authentication
      </h2>

      {loading ? (
        <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>Checking your security settings…</p>
      ) : enabled ? (
        <>
          <p style={{ fontSize: 13, color: 'var(--success)', margin: '4px 0 12px' }}>
          ...
          <form onSubmit={disable} style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <input
              ...
              style={{
                ...
                color: 'var(--text)', borderRadius: 8, padding: '8px 10px', fontSize: 14, letterSpacing: 4,
        ...
        <TwoFactorSetupForm variant="light" onEnabled={() => { setShowSetup(false); setEnabled(true); }} />
      ) : (
        <>
