...
- Production startup fails closed unless database, HTTPS app URL, signing credentials, and explicit CORS are configured. RLS remains a separate database migration gate and must be verified before enabling it in a serverless deployment.
